...
class TSNDataSet(data.Dataset):
    def __init__(self, root_path, list_file, num_dataload,
                 ...
                 force_grayscale=False, random_shift=True, test_mode=False, triple=0, id_list=None):
    ...
    def get(self, record, indices):
        if "hmd" in record.path or "ucf101" in record.path:

            process_data = torch.load(
                os.path.join(".", record.path.replace("I3D-feature-pretrain", 'cat_I3D-feature-pretrain') + ".pt"),
                weights_only=True)

        else:
            process_data = torch.load(os.path.join(".", record.path + ".pt"))
        process_data = process_data[indices]
